...
/* ═══════════════════════════════════════════════════════════
   EXPORT
═══════════════════════════════════════════════════════════ */
// Provenance stamp for generated code (cc = comment char).
function _provenanceComment(fit, cc) {
  const ver = (typeof APP_VERSION !== 'undefined') ? APP_VERSION : '?';
  return [
    `${cc} Generated by Curve Fitting Studio v${ver} — ${new Date().toISOString().slice(0, 10)}`,
    `${cc} Model: ${fit.model}  ·  Solver: ${(fit.algo || 'lm').toUpperCase()}  ·  Weights: ${fit.weightMode || 'none'}`,
    `${cc} Parameter values below are the converged CFS fit (multi-start is seeded, so fits reproduce).`,
    ``,
  ];
}
